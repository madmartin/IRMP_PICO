...
static const char *DESCRIPTION    = tr("show vdr status on irmp device");
...
static bool open_irmp(const char *devicename) {
	...
	if (irmpfd == -1) {
		dsyslog("statusleds2irmp: error opening irmp device: %s\n",strerror(errno));
...
static void read_irmp() {
	...
	if (retVal < 0) {
		dsyslog("statusleds2irmp: read error\n");
		printf("read error\n");
        } /*else {
                printf("read %d bytes:\n\t", retVal);
...
static void write_irmp() {
	...
	if (retVal < 0) {
		dsyslog("statusleds2irmp: write error\n");
		printf("write error\n");
        } /*else {
...
static void send_report(uint8_t led_state, const char *device) {
	if (stop)
	    return;
	open_irmp(device != NULL ? device : "/dev/irmp_pico");
...
class cPluginStatusLeds2irmp : public cPlugin {
private:
  cStatusUpdate *oStatusUpdate;
  cRecordingPresignal *oRecordingPresignal;
public:
  cPluginStatusLeds2irmp(void);
  virtual ~cPluginStatusLeds2irmp() override;
...
// --- cMenuSetupStatusLeds2irmp -------------------------------------------------------

class cMenuSetupStatusLeds2irmp : public cMenuSetupPage {
private:
  int iNewOnDuration;
  int iNewOffDuration;
...
public:
  cMenuSetupStatusLeds2irmp(void);
  };

cMenuSetupStatusLeds2irmp::cMenuSetupStatusLeds2irmp(void)
{
...
void cMenuSetupStatusLeds2irmp::Set(void)
{
  int current = Current();
  Clear();
  ...
  // Add ioctl() options
  Add(new cMenuEditIntItem( tr("Setup.StatusLeds2irmp$On time (100ms)"), &iNewOnDuration, 1, 99));
  Add(new cMenuEditIntItem( tr("Setup.StatusLeds2irmp$On pause time (100ms)"), &iNewOnPauseDuration, 1, 99));
  Add(new cMenuEditIntItem( tr("Setup.StatusLeds2irmp$Off time (100ms)"), &iNewOffDuration, 1, 99));

  SetCurrent(Get(current));
}

eOSState cMenuSetupStatusLeds2irmp::ProcessKey(eKeys Key)
{
...
void cMenuSetupStatusLeds2irmp::Save(void)
{
...
void cMenuSetupStatusLeds2irmp::Store(void)
{
  Save();
...
// --- cPluginStatusLeds2irmp ----------------------------------------------------------

cPluginStatusLeds2irmp::cPluginStatusLeds2irmp(void)
{
...
cPluginStatusLeds2irmp::~cPluginStatusLeds2irmp()
{
...
const char *cPluginStatusLeds2irmp::CommandLineHelp(void)
{
...
bool cPluginStatusLeds2irmp::ProcessArgs(int argc, char *argv[])
{
  ...
  int c;
  while ((c = getopt_long(argc, argv, "d:pwi", long_options, NULL)) != -1) {
        switch (c) {
          case 'd':
            //iOnDuration = 1;
            //iOffDuration = 10;
...
{
    dsyslog("statusleds2irmp: Thread started (pid=%d)", getpid());
...
bool cPluginStatusLeds2irmp::Start(void)
{
...
cMenuSetupPage *cPluginStatusLeds2irmp::SetupMenu(void)
{
  // Return a setup menu in case the plugin supports one.
  return new cMenuSetupStatusLeds2irmp;
}

bool cPluginStatusLeds2irmp::SetupParse(const char *Name, const char *Value)
{
...
VDRPLUGINCREATOR(cPluginStatusLeds2irmp); // Don't touch this!
