...
monit:	while(true) {
		...
		if (retValm >= 0) {
			...
			if (inBuf[0] == REPORT_ID_IR) {
				...
				printf("%02hhx%02hhx%02hhx%02hhx%02hhx%02hhx   pass_on_delta_detection_f: %f tmp_delta: %d delta: %d min_delta: %d upper_border: %d same key: %d timeout: %d repeat detected: %d", inBuf[1],inBuf[3],inBuf[2],inBuf[5],inBuf[4],inBuf[6], ((float)(inBuf[58] * 0xFF + inBuf[57]) * 52) / 1000, inBuf[56], inBuf[63], inBuf[62], inBuf[59], inBuf[54], inBuf[61], inBuf[60]);
				printf("\n\n");
			}
		}
	}
